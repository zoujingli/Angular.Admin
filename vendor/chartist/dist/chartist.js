...
(function (window, document, Chartist) {
   ...
   * @param {Number} adminInDegrees Admin of circle segment in degrees
   * @return {Number} Coordinates of point on circumference
   */
  Chartist.polarToCartesian = function (centerX, centerY, radius, adminInDegrees) {
    var adminInRadians = (adminInDegrees - 90) * Math.PI / 180.0;

    return {
      x: centerX + (radius * Math.cos(adminInRadians)),
      y: centerY + (radius * Math.sin(adminInRadians))
    };
  };

  /**
   * Initialize chart drawing rectadmin (area where chart is drawn) x1,y1 = bottom left / x2,y2 = top right
   *
   * @memberof Chartist.Core
   * @param {Object} svg The svg element for the chart
   * @param {Object} options The Object that contains all the optional values for the chart
   * @param {Number} [fallbackPadding] The fallback padding if partial padding objects are used
   * @return {Object} The chart rectadmins coordinates inside the svg element plus the rectadmins measurements
   */
...
(function(window, document, Chartist) {
  ...
  var defaultOptions = {
    ...
    // The start admin of the pie chart in degrees where 0 points north. A higher value offsets the start admin clockwise.
    startAdmin: 0,
  ...
  function createChart(options) {
    ...
    // Calculate end admin based on total sum and current data value and offset with padding
    var center = {
    ...
    for (var i = 0; i < this.data.series.length; i++) {
      ...
      var endAdmin = startAdmin + dataArray[i] / totalDataSum * 360;
      // If we need to draw the arc for all 360 degrees we need to add a hack where we close the circle
      // with Z and use 359.99 degrees
      if(endAdmin - startAdmin === 360) {
        endAdmin -= 0.01;
      }

      var start = Chartist.polarToCartesian(center.x, center.y, radius, startAdmin - (i === 0 || hasSingleValInSeries ? 0 : 0.2)),
        end = Chartist.polarToCartesian(center.x, center.y, radius, endAdmin);

      // Create a new path element for the pie chart. If this isn't a donut chart we should close the path for a correct stroke
      var path = new Chartist.Svg.Path(!options.donut)
        .move(end.x, end.y)
        .arc(radius, radius, 0, endAdmin - startAdmin > 180, 0, start.x, start.y);
      ...
      if(options.showLabel) {
        // Position at the labelRadius distance from center and between start and end admin
        var labelPosition = Chartist.polarToCartesian(center.x, center.y, labelRadius, startAdmin + (endAdmin - startAdmin) / 2),
      ...
      // Set next startAdmin to current endAdmin. Use slight offset so there are no transparent hairline issues
   ...
   *
   * @example
   * // Using donut, startAdmin and total to draw a gauge chart
   * new Chartist.Pie('.ct-chart', {
   *   series: [20, 10, 30, 40]
   * }, {
   *   donut: true,
